...
const output=process.env.BRASSREACH_ARTIFACTS||path.resolve('tests','artifacts','overhaul12');
const executablePath=process.env.CHROME_PATH||'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
await fs.mkdir(output,{recursive:true});
const assert=(condition,message)=>{ if(!condition) throw new Error(message); };
const report={intro:{},audio:{},navigation:[],layouts:[],input:{},story:{},migration:{},consoleErrors:[],failedLocalRequests:[]};
...
assert(transition.awakening&&transition.bloom>.45,'Golden journey bloom did not cover the foreground');
...
report.audio.layers=await page.evaluate(()=>Engine.getAudioDebug().sources);
const openingSounds=await page.evaluate(()=>Engine.getIntroSoundDebug().map(entry=>entry.kind));
assert(openingSounds.includes('journey')&&openingSounds.includes('cover')&&!openingSounds.includes('sparkle'),'Opening does not use the new impact path cleanly');
...
const intraFade=await page.evaluate(()=>({art:Number(getComputedStyle(document.querySelector('#introArtLayer')).opacity),copy:Number(getComputedStyle(document.querySelector('.slide.active .copy')).opacity)}));
assert(intraFade.art>.9&&intraFade.copy<.85,'Intra-Folio transition did not leave the painting stationary while fading copy');
  ...
  for(const [folio,count] of [[0,2],[1,3],[2,2]]){
    for(let passage=0;passage<count;passage++){
      ...
      const metric=await page.evaluate(()=>{
        ...
        return {width:innerWidth,folio:Number(document.querySelector('.book-shell').dataset.folio),passage:Number(document.querySelector('.book-shell').dataset.passage),stageCenter:stage.left+stage.width/2,stageRight:stage.right,copy:[copy.left,copy.top,copy.right,copy.bottom],nav:[nav.left,nav.top,nav.right,nav.bottom],fits:passage.scrollHeight<=passage.clientHeight+2,overflow:document.documentElement.scrollWidth-innerWidth};
      });
      assert(Math.abs(metric.stageCenter-width/2)<1.5,`Stage is not centered at ${width}px`);
      assert(metric.copy[2]<=metric.stageRight-(metric.stageRight-(metric.stageCenter))*.41,`Copy approaches the right-page edge at ${width}px`);
...
const reducedAnimations=await reducedPage.evaluate(()=>({lantern:getComputedStyle(document.querySelector('.intro-lantern-light')).animationDuration,bloom:getComputedStyle(document.querySelector('.intro-golden-bloom')).animationDuration}));
assert(parseFloat(reducedAnimations.lantern)<.1&&parseFloat(reducedAnimations.bloom)<.1,'Reduced motion does not suppress decorative intro animation');
